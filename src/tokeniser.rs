...
fn token_to_char(token: &TokenType) -> Result<char, SourceError> {
    match token {
        ...
        _ => Err(token_err(
            &format!("Unexpected token type! {}", token),
            token,
        )),
    }
}

fn generic_normalise(
    ...
    match created_token {
        ...
        None => Err(token_err(
            &format!("Unterminated {} starting here", token_typename),
            &start_token,
        )),
    }
}
...
fn normalise_declarations(tokens: &mut VecDeque<TokenType>) -> Result<TokenType, SourceError> {
    generic_normalise(
        ...
        |t| !matches!(t, TokenType::Quote(..) | TokenType::Character(..)),
        |s, fname, ln, cn| Ok(TokenType::Declaration(s.clone(), fname, ln, cn)),
    )
}
...
pub fn normalise(mut tokens: VecDeque<TokenType>) -> Result<VecDeque<TokenType>, SourceError> {
    ...
    new_tokens.retain(|t| !matches!(t, TokenType::Whitespace(..) | TokenType::Newline(..)));
...
mod tests {
    ...
    fn tokenise_ignores_comments() {
        ...
        assert_eq!(tokenise("<in>", "# This is a comment!"), vec![]);

        // Line up to that point is tokenised
        assert_eq!(
            tokenise("<in>", "(a) # Comment rest of line"),
            vec![
                TokenType::OpenBracket("<in>".into(), 1, 1),
...
bar # abc\""
    ...
    fn normalise_newline_ends_delcaration() {
        assert_eq!(
            process_into_tokens("<in>", "(let 'a 1 'b").unwrap(),
            vec![
                TokenType::OpenBracket("<in>".into(), 1, 1),
    ...
    fn normalise_newline_ends_number_or_symbol() {
        assert_eq!(
            process_into_tokens("<in>", "(+ foo bar").unwrap(),
            vec![
                TokenType::OpenBracket("<in>".into(), 1, 1),
...
bar\""
        ...
        assert_eq!(
            process_into_tokens("<bla>", "('fo'o)").unwrap(),
            vec![
                TokenType::OpenBracket("<bla>".into(), 1, 1),
        ...
        assert_eq!(
            process_into_tokens("<a>", "(123 a56)").unwrap(),
            vec![
                TokenType::OpenBracket("<a>".into(), 1, 1),
        ...
        assert_eq!(
            process_into_tokens("<a>", "0xcafe").unwrap(),
            vec![TokenType::Integer(0xcafe, "<a>".into(), 1, 1)]
        );

        // Whitespace removed
        assert_eq!(
            process_into_tokens("<a>", "(              )").unwrap(),
            vec![
                TokenType::OpenBracket("<a>".into(), 1, 1),
        ...
        assert_eq!(
            process_into_tokens("<b>", "'foo\nbar\nabc").unwrap(),
            vec![
                TokenType::Declaration("foo".into(), "<b>".into(), 1, 1),
